...
class GSP_Email {
    ...
    public static function send_conversion_status($to, $status, $amount, $conversion_type) {
        ...
        $subject = $status === 'approved' 
            ? "Conversion Approved - GlobalSwiftPay" 
        ...
        if ($status === 'approved') {
            $message = "Hello,<br><br>This is to inform you that the conversion made from " . esc_html($to) . " of $" . number_format($amount, 2) . " has been approved. You will be credited shortly.<br><br>Thank You for using Globalswiftpay2.0 Platform.";
        } else {
            $message = "Hello,<br><br>We regret to inform you that your recent conversion of $" . number_format($amount, 2) . " to " . esc_html($type_label) . " has been declined due to invalid security phrase.<br><br>Kindly generate your account personal security phrase from the platform and then input the generated security phrase during your conversion process in order for your next conversions to be approved by the system.<br><br>Kindly message the support chat on the platform for any inquiries, thank you.";
        }
    ...
    /**
     * Send conversion request received email to user
     */
    public static function send_conversion_request_received($to, $amount, $conversion_type) {
        $type_label = ucfirst($conversion_type);
        if ($conversion_type === 'btc') {
            $type_label = 'Bitcoin';
        } elseif ($conversion_type === 'usdt') {
        ...
        $subject = "Conversion Request Received - GlobalSwiftPay";
        
        $message = "Your conversion to " . esc_html($type_label) . " of $" . number_format($amount, 2) . " is being reviewed and if you inputted the right security phrase for your account, it would be approved and you'd be credited shortly.";
        
        ob_start();
        ?>
        <h2 style="color: #333; margin-bottom: 10px;">Conversion Request Received</h2>
        <p><?php echo esc_html($message); ?></p>
        <div class="amount">$<?php echo number_format($amount, 2); ?></div>
        <p>Conversion Type: <strong><?php echo esc_html($type_label); ?></strong></p>
