...
class GSP_Transactions {
    ...
    public static function get_all_deposits($status = '') {
        ...
        // Flush any cache to ensure fresh data
        wp_cache_flush();
    ...
    public static function update_deposit_status($deposit_id, $status, $notes = '') {
        ...
        // If update failed, try direct query as fallback
        if ($result === false) {
            $wpdb->query($wpdb->prepare(
                "UPDATE $table SET status = %s, admin_notes = %s, updated_at = %s WHERE id = %d",
                $status,
                $notes,
                current_time('mysql'),
                $deposit_id
    ...
    public static function update_conversion_status($conversion_id, $status, $notes = '') {
        ...
        // Perform the update with explicit format specifiers
        $result = $wpdb->update(
            $table,
            array(
                'status' => $status,
